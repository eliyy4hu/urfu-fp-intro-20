...
howManyDigits n = if tenCount == 0 then "single" else (if hundredCount == 0  then "two-digit" else "three-digit or more")
  where
    tenCount = div (abs n) 10
    hundredCount = div tenCount 10
...
-}
digitsCount :: Int -> Int
digitsCount n = if tenCount == 0 then 1 else 1 + digitsCount(tenCount)
  where
    tenCount = div (abs n) 10
